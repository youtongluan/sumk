...
public class DemoUserDao {
	...
	public void select() throws ParseException{
		...
		list=DB.select().tableClass(DemoUser.class)
				.addEqual(MapBuilder.create("id", 10000).put("age", 16).toMap())
				.addEqual(MapBuilder.create("id", 20000).toMap())
				.queryList();
		...
		System.out.println("相当于select id demouser where (name=kkk and age=12) or (name=第1个 and age=1) order by lastupdate,age desc limit 0,10");
		DB.select(obj)
			.selectColumns("id")
				.addEqual(MapBuilder.create("name", "第1个").put("AGE", 1).toMap())
				.OrderByAsc("lastupdate")
				.OrderByDesc("age")
				.limit(10)
				.queryList();
