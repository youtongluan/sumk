...
public class RedisPool {
	...
	public static Redis get(JedisPoolConfig config, String host, RedisParamter p) throws Exception {
		...
		return redis(config, host.trim(), p.getPort(), p.getTimeout(), p.getPassword(), p.getDb(), p.getTryCount());
	}

	private static Redis redis(JedisPoolConfig config, String host, int port, int timeout, String password, int db,
		...
		if (redis == null) {
			synchronized (RedisPool.class) {
				...
				JedisPool pool = create(config, host, port, timeout, password, db);
