...
public class TableGenerator implements Plugin{
	...
	private void generoate(PojoMeta pm) throws Exception {
		...
		if(pm.isSoftDelete()){
			...
			joiner.item().append(name(pm.softDelete.columnName))
				.append(' ').append(softType==String.class?"CHAR(1)":dbType(softType))
				.append(" DEFAULT ");
			...
			joiner.append(" COMMENT '")
				.append(AppInfo.get("sumk.db.generator.softdelete.comment",comment))
				.append('\'');
		}
