...
public class SqlSessionFactory {
	...
	public Map<String,Map<String,Integer>> status(){
		...
		for(DataSource datasource:set){
			if(!BasicDataSource.class.isInstance(datasource)){
				Log.get(this.getClass(),25345).info("ds.class({}) is not instance form BasicDataSource",datasource.getClass().getName());
				continue;
			}
	...
	public static SqlSessionFactory get(String dbName){
		try {
			...
			Log.get(SqlSessionFactory.class,"create_error").error(e);
	...
	private String mapperPartern(){
		return AppInfo.getDBRoot(PathMatchingResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX+"db/")+db+"/**/*.xml";
	}
