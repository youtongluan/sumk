...
public class SimpleBeanUtil {
	...
	public static void setProperty(Object bean,Method m,String value) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
		if(value==null){
			Log.get(SimpleBeanUtil.class,"setProperty").debug("{}因为值为null被过滤掉",m.getName());
			return;
		}
		value=value.trim();
		if(value.isEmpty()){
			Log.get(SimpleBeanUtil.class,"setProperty2").debug("{}因为值为空被过滤掉",m.getName());
			return;
		}
	...
	/**
	 * 设置对象属性，
	 * setXX方法不能重名，并且参数只有一个。
	 * 如果setXX方法不存在，就忽略map中的该值
	 * @param bean
	 * @param map
	 */
