	...
	public void resolve(Class<?> clz) throws IOException, ClassNotFoundException {
		...
		for (final Method m : clz.getMethods()) {
			...
			if(ActionHolder.getActionInfo(soaName)!=null){
				Log.get("SYS.13").error(soaName+"已经存在");
				continue;
			}
			if(obj==null){
			...
			if (argSize == 0) {
				Log.get("SYS.15").trace("{}解析出来的空方法：{}()" , clz.getName(),m.getName());
			...
			cr.accept(cv, 0);
			MethodInfo mInfo=new MethodInfo(m, cv.argNames
					.toArray(new String[0]), cv.descriptor,cv.signatures.toArray(new String[0]));
			Class<?> argClz=AsmUtils.CreateArgPojo(clz.getName(), mInfo);
			HttpHolder.putActInfo(soaName,new HttpInfo(obj,m,argClz,mInfo.getArgNames(),m.getParameterTypes(),act,upload));
		}
...
class MethodInfoClassVisitor extends ClassVisitor {
	...
	public MethodVisitor visitMethod(final int access, final String name,
		...
		if (!name.equals(m.getName())
				|| !AsmUtils.sameType(args, m.getParameterTypes())) {
		...
		return new MethodVisitor(Vars.ASM_VER, v) {
			...
			public void visitLocalVariable(String name, String desc,
				...
				super.visitLocalVariable(name, desc, signature, start, end,
						index);
				int argSize = m.getParameterTypes().length;
				if ("this".equals(name) || argNames.size() >= argSize
						|| argNames.contains(name)) {
					return;
				}
				int k = argNames.size();
				if (!args[k].getDescriptor().equals(desc)) {
					
					Log.get("SYS.20").error("###当前参数的类型应该是"
							+ args[k].getDescriptor() + ",但实际却是" + desc);
					return;
				}
				argNames.add(name);
