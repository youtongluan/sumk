...
public final class HttpInfo {
	...
	public HttpInfo(Object obj, Method m, Class<?> argClz,String[] argNames, Class<?>[] argTypes, Web action,Upload upload) {
		super();
		this.obj = obj;
		this.m = m;
		this.argClz=argClz;
	...
	public Object invokeByOrder(String... args) throws Exception{
		...
		if(args.length!=argTypes.length){
			SystemException.throwException(12013, m.getName()+"需要传递的参数有"+argTypes.length+"个，实际传递的是"+args.length+"个");
		}
		for(int i=0,k=0;i<params.length;i++){
			if(ActionContext.class.isInstance(getArgTypes()[i])){
